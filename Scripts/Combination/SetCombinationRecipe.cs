...
public class SetCombinationRecipe : MonoBehaviour
{
    public static SetCombinationRecipe Instance { get; private set; }
    ...
    public void Create()
    {
        ...
        if (selectedItem.itemType == ItemType.Ingredient)
        {
            ...
            if (InventoryManager.Instance.CheckItem(items))
            {
                ...
                StartCoroutine(CreateItemAfterDelay(2.0f, selected, items)); // 3초 후에 CreateItemCoroutine 코루틴을 시작합니다.
            }
            else
            {
