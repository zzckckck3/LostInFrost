...
namespace ECM.Controllers
{
    ...
    public class ModifiedCharacterController : MonoBehaviour
    {
        ...
        private float RunOrWork = 1f;
        private bool _restoreVelocityOnResume = true;
        protected ChangeAnimatorLayer changeAnimatorLayer;
        ...
        public Vector3 moveDirection
        {
            ...
            { 
                if(RunOrWork == 1f)
                {
                    _moveDirection = Vector3.ClampMagnitude(value, 1.0f);
                    
                }else if(RunOrWork == 2f)
                {
                    _moveDirection = Vector3.ClampMagnitude(value, 2.0f);
                }
        ...
        public virtual void RotateTowardsMoveDirection(bool onlyLateral = true)
        {
        ...
        {
            if (useRootMotion && applyRootMotion && useRootMotionRotation && false)
            {
        ...
        protected virtual void HandleInput()
        {
            ...
            if (Input.GetKey(KeyCode.LeftShift) && changeAnimatorLayer.isBaseLayer && UserStatusManager.Instance.CanRun)
            {
                UserStatusManager.Instance.IsRun = true;
                RunOrWork = 2f;
            }
            else
            {
                UserStatusManager.Instance.IsRun = false;
                RunOrWork = 1f;
            }
            ...
            moveDirection = RunOrWork * moveDirection;
        ...
        public virtual void Awake()
        {
            ...
            changeAnimatorLayer = GetComponent<ChangeAnimatorLayer>();
        }

        public virtual void FixedUpdate()
        {
            if (SceneManager.GetActiveScene().name.Equals("Home") || pv.IsMine)
            {
